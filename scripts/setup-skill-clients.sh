...
# into .cursor/skills/, .claude/skills/, .codex/skills/, .vscode/skills/, .opencode/skills/ so Cursor, Claude, Codex, VS Code, openCode auto-discover them.
...
# Optional: --clients cursor,claude,codex,vscode,opencode to link only to those (default: all)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --clients)
      ...
      ;;
    -h|--help)
      echo "Usage: $0 [--copy] [--clients cursor,claude,codex,vscode,opencode] [skill1 [skill2 ...]]"
...
client_opencode_dir=".opencode/skills"
...
CLIENT_LIST=""
[ ${#CLIENT_FILTER[@]} -eq 0 ] && CLIENT_LIST="Cursor, Claude, Codex, VS Code, openCode" || CLIENT_LIST=$(IFS=,; echo "${CLIENT_FILTER[*]}")
...
for name in "${SKILL_NAMES[@]}"; do
  ...
  if client_enabled "opencode"; then
    client_dir="${REPO_DIR}/${client_opencode_dir}"
    link_path="${client_dir}/${name}"
    mkdir -p "$client_dir"
    if [ -L "$link_path" ]; then rm -f "$link_path"; fi
    if [ -e "$link_path" ]; then
      echo "  Skip .opencode/skills/${name} (already exists and is not a symlink)"
    elif [ "$USE_COPY" = true ]; then
      cp -R "$src" "$link_path"
      echo "  Copied ${name} -> .opencode/skills/"
    else
      ln -sf "../../skills/${name}" "$link_path"
      echo "  Linked ${name} -> .opencode/skills/"
    fi
  fi
