...
├── Projects/
│   ├── ThingsToDo/            # Default: Loose tasks
│   └── ThingsToLearn/         # Default: Learning topics
├── $VAULT_PATH/templates/     # System Templates (clean, minimal)
└── Daily/                     # Daily Notes
```
...
- Creates `Projects/[Name]/AGENT.md` from template
...
- **Action**:
    1.  **Create Note**: Create today's Daily Note using `$VAULT_PATH/templates/daily_note_template.md`.
    2.  **Calendar Scan**: Retrieve today's calendar events (if calendar integration available)
        - Extract calendar events and populate `## 📅 Calendar` section
        - Identify project names/tags from event titles -> Add to `## 🚀 Targeted Projects`
        - Note any meetings or time blocks for the day
    3.  **Rollover**: Scan yesterday's note for incomplete tasks `[ ]` -> Copy to `## 📋 Rolled Over Tasks`.
    4.  **Critical Check**: Scan all `Projects/*/tasks.md` files for tasks with `@Today` or `@YYYY-MM-DD` (today's date) -> Copy to `## 🚨 Critical / Due Today` **with project tag** (e.g., `#things-to-do`, `#things-to-learn`).
    5.  **Upcoming Tasks**: Scan all `Projects/*/tasks.md` files for tasks with future dates `@YYYY-MM-DD` -> Copy to `## 📅 Upcoming Tasks` **with project tag** (show recurrently until due date).
    6.  **Undated Tasks**: Scan all `Projects/*/tasks.md` files for incomplete tasks without `@` date tags -> Copy to `## 📌 Undated Tasks` **with project tag** (show daily until completed or dated).
...
- **Action**:
    ...
    - **Loose Tasks (Inbox)**: Copy to `Projects/ThingsToDo/tasks.md` under Inbox section.
    ...
    - **Learning Items**: Copy to `Projects/ThingsToLearn/tasks.md` (tasks only, not catalog entries).
    - **File Notes**: Extract notes under `## Notes` or with specific tags -> Copy to `Projects/[Name]/notes.md`.
...
- **Action**:
    ...
    2.  **Instantiate**: Copy templates to create:
        - `Projects/[Name]/AGENT.md` (from `$VAULT_PATH/templates/project_agent_template.md`)
        ...
        - `Projects/[Name]/project.md` (individual project template)
        - `Projects/[Name]/tasks.md` (from `$VAULT_PATH/templates/project_tasks_template.md`)
        - `Projects/[Name]/meetings.md` (from `$VAULT_PATH/templates/meeting_note_template.md`)
        - `Projects/[Name]/team_members.md` (individual project template)
        - `Projects/[Name]/notes.md` (individual project template)
...
- **Action**:
    1.  **Read** `$VAULT_PATH/templates/system_agent_template.md` (Source of Truth).
...
- **Action**:
    1.  **Locate Project**: Find `Projects/[ProjectName]/` folder.
    2.  **Load Context**: Read `Projects/[ProjectName]/AGENT.md` to understand project-specific rules and context.
    3.  **Show Overview**: Display project status, phase, objectives from `project.md`.
    4.  **List Tasks**: Show critical (@Today, due dates), in-progress, and upcoming tasks from `tasks.md`.
    5.  **Recent Activity**: Display last 3-5 meetings from `meetings.md`.
    6.  **Team Status**: Show team members and collaboration info from `team_members.md`.
    7.  **Blockers/Risks**: Highlight any blockers or risks from `notes.md`.
    8.  **Activate Context**: Apply project-specific AGENT.md rules for all subsequent interactions.
- **Result**: AI assistant operates with full project context. Project-specific workflows and conventions are active.
...
Projects/ThingsToDo/tasks.md (Inbox section)
...
## After "Digest My Day" → Copied To
Projects/WebsiteRedesign/tasks.md
...
Copied to `Projects/ThingsToDo/meetings.md`:
...
Copied to `Projects/ThingsToLearn/notes.md`:
...
|---------|-----|-------------|--------|----------|
| **My Custom Project** | `#custom` | Special project | Active | `Projects/MyCustom/` |
```
...
1. **Tagged Tasks** (`#project-name`): Go to `Projects/[Name]/tasks.md`
2. **Untagged Tasks**: Go to `Projects/ThingsToDo/tasks.md` (Inbox)
3. **Learning Items** (no project tag): Go to `Projects/ThingsToLearn/tasks.md`
...
2. Copy to `Projects/[Name]/meetings.md` based on `#project-name` tag
3. Extract action items -> Copy to `Projects/[Name]/tasks.md`
...
- **Check**: Does `$VAULT_PATH/templates/daily_note_template.md` exist?
